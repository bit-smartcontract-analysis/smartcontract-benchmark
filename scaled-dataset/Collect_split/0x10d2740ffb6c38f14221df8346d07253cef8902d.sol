// hevm: flattened sources of src/strategies/sushiswap/strategy-sushi-eth-usdt-lp.sol
...
////// src/strategies/strategy-sushi-farm-base.sol
// SPDX-License-Identifier: MIT
/* pragma solidity ^0.6.7; */
...
abstract contract StrategySushiFarmBase is StrategyBase {
    ...
    function deposit() public override {
        uint256 _want = IERC20_1(want).balanceOf(address(this));
        if (_want > 0) {
            IERC20_1(want).safeApprove(masterChef, 0);
            IERC20_1(want).safeApprove(masterChef, _want);
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _sushi = IERC20_1(sushi).balanceOf(address(this));
        if (_sushi > 0) {
            // 10% is locked up for future gov
            uint256 _keepSUSHI = _sushi.mul(keepSUSHI).div(keepSUSHIMax);
            IERC20_1(sushi).safeTransfer(
                IController(controller).treasury(),
                _keepSUSHI
            );
        ...
        uint256 _weth = IERC20_1(weth).balanceOf(address(this));
        if (_weth > 0) {
            _swapSushiswap(weth, token1, _weth.div(2));
        }

        // Adds in liquidity for ETH/DAI
        _weth = IERC20_1(weth).balanceOf(address(this));
        uint256 _token1 = IERC20_1(token1).balanceOf(address(this));
        if (_weth > 0 && _token1 > 0) {
            IERC20_1(weth).safeApprove(sushiRouter, 0);
            IERC20_1(weth).safeApprove(sushiRouter, _weth);
            ...
            // Donates DUST
            IERC20_1(weth).transfer(
                IController(controller).treasury(),
                IERC20_1(weth).balanceOf(address(this))
            );
...
////// src/strategies/sushiswap/strategy-sushi-eth-usdt-lp.sol
// SPDX-License-Identifier: MIT
/* pragma solidity ^0.6.7; */
